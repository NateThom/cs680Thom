...
{  
  ...
  if(!loadOBJ(Vertices, Indices, o_path, f_path))
  {
...
bool Object::loadOBJ(std::vector<Vertex> &out_vertices, std::vector<unsigned int> &out_indices, std::string o_path, std::string f_path)
{
  ...
  for(int i = 0; i < meshes.size(); i++){
    for(int j = 0; j < meshes[i]->mNumFaces; j++){
      ...
      for(int k = 0; k < 3; k++){
        ...
        Vertex *temp = new Vertex(glm::vec3(aiVector.x, aiVector.y, aiVector.z), material_color[i]);
